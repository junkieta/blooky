...
drip.registerEnhancer((e) => {
    const prop = e.prop;
    if(!PROP_OBSERVERS.has(prop)) return e;
    const update = e.update;
    return {
        ...e,
        update: (v) => {
            PROP_OBSERVERS.get(prop)!.forEach((f)=>f(v, prop()));
            update(v);
        }
    };
});
...
const bind = <T>(prop:Prop<T>) => (updater:(v:T)=>boolean) => {
    if(!PROP_OBSERVERS.has(prop))
        PROP_OBSERVERS.set(prop, new Set());
    PROP_OBSERVERS.get(prop)!.add(function _(v:T) { if(!updater(v)) PROP_OBSERVERS.get(prop)?.delete(_); });
};
...
const bind_node_prop = <T extends JSHTMLNodeSource>(prop:Prop<T>) => function f(p:[Node,Node]) {
    bind(prop)((v)=>{
        if(!p.every((n)=>n.isConnected)) return false;
        p= update_range(p)(v);
        return true;
    })
}
...
const bind_attr_prop = (prop:Prop<JSHTMLAttrSource>) => ([e,n]:[HTMLElement,string]) => {
    bind(prop)((v)=>{
        if(e.isConnected) update_attr(e)([n,v] as T_ATTRSET);

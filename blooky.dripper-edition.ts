...
const DRIPPER_STREAM_RELATIONS = new WeakMap<Dripper<any>, Set<Stream<any>>>();

type Dripper<A> = (v:A) => Effect

const dripper = <A>() => {
    
    const f: Dripper<A> = (v:A)=> 
        [...DRIPPER_STREAM_RELATIONS.get(f) as Set<Stream<A>>]
        .map(flowLazy(v))
        .reduce((effect, [_,observers,updates])=>{
            effect.observers.push(...observers);
            effect.updates.push(...updates);
            return effect;
        }, {observers:[],updates:[]} as Effect);
    
    DRIPPER_STREAM_RELATIONS.set(f, new Set<Stream<A>>());
    return f;
}
...
const flowingStateToEffect = ([_,observers,updates]:FlowingState) : Effect => ({observers,updates});
...
function drip<A>(s: Stream<A>) {
    const dripFn = (v: A) => {
        execute(run(dripEffectToFxNodeSaga(flowingStateToEffect(flowLazy(v)(s)))));
    };
    // drip関数に、atomicモードで実行するメソッドを生やす
    dripFn.atomic = (v: A) => {
        execute(run(dripEffectToFxNodeAtomic(flowingStateToEffect(flowLazy(v)(s)))));
    };
    return dripFn as any; // 型推論を助けるためにanyにキャストするが、関数のシグネチャが型安全性を保証する
};

// drip-Atomicモード：全処理を一つのfx.callにまとめる
const dripEffectToFxNodeAtomic = ({observers,updates}: Effect) => fx.call(() => {
...
const dripEffectToFxNodeSaga = ({observers,updates}: Effect) => {
...
export {dripper,stream,isStream,countReferences,hasReferences,clear,hold,accum,lift,merge,pipe,map,filter,listen,drip,shed,moments};
